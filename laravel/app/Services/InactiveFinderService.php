...
class InactiveFinderService
{
    ...
    public function search(int $serverId, int $x, int $y, int $page = 1): array
    {
        $page = max(1, min($page, self::MAX_PAGES));

        $matchCount = (int) DB::table('villages')
            ->where('server_id', $serverId)
            ->where('days_without_change', '>=', 1)
            ->count();
